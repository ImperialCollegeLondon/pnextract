...
{
	_minRp=1.75;
	if (cfg.getVar(_minRp, "Rnoise"+toStr(0)) || cfg.getVar(_minRp, "minRPore") || cfg.getVar(_minRp, "Rnoise")) cout<< " minimum pore radius: " << _minRp <<endl;
	else  cout<<" keyword \"minRPore\" not found, default value ("<<_minRp<<") will be used"<<endl;

	_clipROutx=0.05;
	_clipROutyz=0.98;
	_midRf=0.7;
	_MSNoise=1.0*_minRp+1.0;
	_lenNf=0.6;
	_vmvRadRelNf=1.1;
	_nRSmoothing=3;
	_RCorsnf=0.15;
    ...
    if (cfg.getData(keywrdData, "medialSurfaceSettings"+toStr(0)) || cfg.getData(keywrdData, "medialSurfaceSettings"))
	 {
	 	keywrdData  >>_clipROutx >>_clipROutyz  >>_midRf >>_MSNoise  >>_lenNf >>_vmvRadRelNf >>_nRSmoothing >>_RCorsnf >>_RCorsn;
	 }


	 cout<<" minRPore     : "<< _minRp<<";\n";
...
{ ///  Build voxelspace -- memory for void/active voxels
	...
	///. Warning vxlSpace should be built in order of z++ y++ x++
...
void medialSurface::attachVoxelspace()
...
void medialSurface::paradoxremoveincludedballI() //to remove the included maximal bals
		...
		for (int a = -ex; a <=  ex; ++a)
		{
			...
			for (int b = -ey; b <=  ey; ++b)
			{
				...
				for (int c = -ez; c <=  ez; ++c)
				{
					...
					if ((vj != NULL) && (vj->ball) && (vj != vi)) //if find ball centers
					{
						const float rj = vj->R;
						if(rj <= ri)
						{
...
{/// Refines the maximal-sphere location, by moving it uphil the gradient of the distance-map in the same voxel
	const voxel* vi = vxl(b_i->fi+_pp5, b_i->fj+_pp5, b_i->fk+_pp5);
	vec3 disp(0.0, 0.0, 0.0);
	{
		...
		if (vjm && vjp)
		{
			float gp = vjp->R-vi->R;
			float gm = vi->R-vjm->R;
			if(abs(gp-gm)>0.01)  disp.x = max(-0.49, min(0.49, -0.5*(gp+gm)/(gp-gm))); //d_geq0 = -0.5+(0.5+0.5)/(gp-gm)*(0.0-gm);
		}
	}
	{
		...
		if (vjm && vjp)
		{
			float gp = vjp->R-vi->R;
			float gm = vi->R-vjm->R;
			if(abs(gp-gm)>0.01)  disp.y = max(-0.49, min(0.49, -0.5*(gp+gm)/(gp-gm))); //d_geq0 = -0.5+(0.5+0.5)/(gp-gm)*(0.0-gm);
		}
	}
	{
		...
		if (vjm && vjp)
		{
			float gp = vjp->R-vi->R;
			float gm = vi->R-vjm->R;
			if(abs(gp-gm)>0.01)  disp.z = max(-0.49, min(0.49, -0.5*(gp+gm)/(gp-gm))); //d_geq0 = -0.5+(0.5+0.5)/(gp-gm)*(0.0-gm);
		}
	}
	if(b_i!=b_i->boss)
	{
		vec3 BosKidVec=*b_i - *(b_i->boss);	
		disp -= 0.95*((BosKidVec&disp)/(magSqr(BosKidVec)+1.0e-12))*BosKidVec;
	}
...
{/// Refines the maximal-sphere location, by moving it uphil the gradient of the distance-map, potentially relocates to new voxels
	...
	vec3 disp(0.0, 0.0, 0.0), grad(0.0, 0.0, 0.0);

	{
		...
		if (vjm && vjp)
		{
			...
			float gm = vi->R-vjm->R;
			grad.x = 0.5*(gp+gm);
			if(abs(gp-gm)>0.01)  disp.x = max(-0.59, min(0.59, -0.5*(gp+gm)/(gp-gm))); //d_geq0 = -0.5+(0.5+0.5)/(gp-gm)*(0.0-gm);
		}
	}
	{
		const voxel* vjm = vxl(vi->i, vi->j-1, vi->k);
		...
		if (vjm && vjp)
		{
			...
			float gm = vi->R-vjm->R;
			grad.y = 0.5*(gp+gm);
			if(abs(gp-gm)>0.01)  disp.y = max(-0.59, min(0.59, -0.5*(gp+gm)/(gp-gm))); //d_geq0 = -0.5+(0.5+0.5)/(gp-gm)*(0.0-gm);
		}
	}
	{
		const voxel* vjm = vxl(vi->i, vi->j, vi->k-1);
		...
		if (vjm && vjp)
		{
			...
			float gm = vi->R-vjm->R;
			grad.z = 0.5*(gp+gm);
			if(abs(gp-gm)>0.01)  disp.z = max(-0.59, min(0.59, -0.5*(gp+gm)/(gp-gm))); //d_geq0 = -0.5+(0.5+0.5)/(gp-gm)*(0.0-gm);
		}
	}
	disp+=1.4*grad;
	...
	disp/=(0.55*mag(disp)+0.05);
		...
		if(vxlj && vi!=vxlj && vxlj->R>vi->R && vxlj->ball==NULL )
		{
				bi->fi=vxlj->i-_mp5;
				bi->fj=vxlj->j-_mp5;
				bi->fk=vxlj->k-_mp5;
				bi->R=vxlj->R;
				bi->vxl->ball=NULL;		bi->vxl=vxlj;
				vxlj->ball=bi;
				//++nrelocations;
		}
		 ...
		 { ++nOverlap;
			...
			for (int ii=0;ii<vi->nKids;++ii)
			{
				...
				for (int jj=0;jj<vj->nKids;++jj)
				  if(notNeiAcceptAsNei(vj, vii,vj->kids[jj]) ) ///. vj is the bigger here
					 ++nAdoptNeis;
			...
			{medialBall* tmp=vi; vi=vj; vj=tmp;}	///. i bigger
			for (int ii=0;ii<vi->nKids;++ii)
			{
				medialBall* vii=vi->kids[ii];
				int nAdoptNeis = vii->nNeis;
...
{/// Select higher level ball in maximal-ball hirachy
	...
	double wsinv=1.0/(rilim+rjlim);
	//  double wi(dSqr+rjlimlim-rlim), wj(dSqr+rlim-rjlimlim);
	voxel* middlevxl=vxl( wsinv*(vi->fi*rjlim+vj->fi*rilim)+_pp5, wsinv*(vi->fj*rjlim+vj->fj*rilim)+_pp5, wsinv*(vi->fk*rjlim+vj->fk*rilim)+_pp5 );
	if ( middlevxl && middlevxl->R>min(ri,rj)*_midRf-0.5 && 1.01*sqrt(dSqr)<ri+rj+1.0+1.0*noise )
	{


		if (vj->boss == vj)  ///. vj is alone
		{	if (vi->mastrVxl() != vj)
			{
				if (ri >= rj)       				vj->boss = vi;
				else if (vi->boss->R <= rj)		vi->boss = vj;
				else if (ri >= rj-noise && ri*_vmvRadRelNf+1.0*noise>=rj) vj->boss = vi;
		}	}
		else if (vi->boss == vi)  ///. vi is alone
		{   if (vj->mastrVxl() != vi)
			{
				if (rj>=ri)         				vi->boss = vj;
				else if (vj->boss->R <= ri)		vj->boss = vi;
				else if (rj>=ri-noise && rj*_vmvRadRelNf+1.0*noise>=ri) vi->boss = vj;
		}	}


		medialBall* mvi=vi->mastrVxl();
		medialBall* mvj=vj->mastrVxl();
		if (mvi != vj && mvj != vi)
		{
		 ...
		 if (mvi==mvj)
		 {
			...
			if ( leveli+1 < levelj &&
					(vj->boss->R-vj->R+2.0*noise)/(dist(vj->boss,vj)+0.25) <  (vi->R-vj->R+2.0*noise+0.01)/(dist(vi,vj)+0.2)
				) vj->boss = vi;
			else if ( leveli > levelj+1 &&
							(vi->boss->R-vi->R+2.0*noise)/(dist(vi->boss,vi)+0.25) <  (vj->R-vi->R+2.0*noise+0.01)/(dist(vj,vi)+0.2)
						) vi->boss = vj;
			else //if (leveli == levelj)
			{
				     if ( leveli > levelj && (vi->boss->R-vi->R+2.0*noise)/(dist(vi->boss,vi)+1.2) <  (vj->R-vi->R+2.0*noise)/(dist(vj,vi)+1.3)
						&& !vj->inParents(vi) ) vi->boss = vj;
				else if ( leveli < levelj && (vj->boss->R-vj->R+2.0*noise)/(dist(vj->boss,vj)+1.2) <  (vi->R-vj->R+2.0*noise)/(dist(vi,vj)+1.3)
						&& !vi->inParents(vj) ) vj->boss = vi;
				else if(middlevxl && middlevxl->R>=0.45*(ri+rj)-1.0 && sqrt(dSqr)<(ri+rj)*0.5+2.0 )
					makeFriend(vi,vj);
			}
			if(vi->mastrVxl() != vj->mastrVxl()) {cout<<"sdsdsds"<<endl; exit(-1);}
			
		 }
		 else //if (mvi!=mvj)
		 {

			if (distSqr(mvi,mvj) <= _lenNf*(0.5*(mvi->R+mvj->R)+2.0*noise)*(0.5*(mvi->R+mvj->R)+2.0*noise))
			{ ///. possibly merge the two clusters
			   ...
			   if (mvj->R < _vmvRadRelNf * vj->R +noise &&  mvj->R < _vmvRadRelNf * vi->R +noise && mvj->R < _vmvRadRelNf * vi->boss->R +noise)
			   {
				  ...
				  if(vj->boss==vj && vi->mastrVxl()!=vj)   vj->boss = vi;  ///. vj is alone
			   }

			}

			if (vi != vj->boss)  ///. if didn't merge
			{ ///. fine tune the boundary
				...
				float distAvg = dist(mvj,mvi)*1.0+0.5*noise;
...
{/// search for higher level ball in the maximal-ball hirachy
	...
	{	float  ey = y+sqrt(ripp*ripp-(xpa-x)*(xpa-x));
			...
			{	float ez = z+sqrt(ripp*ripp-(xpa-x)*(xpa-x)-(y-ypb)*(y-ypb));
				...
				{voxel* vj = this->vxl(xpa, ypb, zpc);///. First we should have sorted the voxels and chosen the biggest ball or the closest ball
					if ((vj != NULL) && vj->ball && (*vi != *vj) ) // ball centers are found
					{ //--------------------------------------------------------
...
{/// Main  driver for creating the medialSurface. Create distance map, maximal-spheres, and their hirarchy (medial-surface connectivity)

//	mediaAxes medAxis(*this, minRP, clipOutSideBallFraction, clipOutSideBallFraction*0.5+0.0);


	buildvoxelspace(); //!  build voxelspace

	attachVoxelspace();//! attach voxel* (from voxelspace) to image (segs_[][]s)


	calc_distmaps();  //!  create and assign metaball to voxelspace



   for (int i=0;i<_nRSmoothing;++i)        smoothRadius();  //!  (slightly) smooth distance map if requested


	paradox_pre_removeincludedballI(); ///. remove balls inside other balls

	paradoxremoveincludedballI(); ///. remove balls inside other balls
	...
	const std::vector<medialBall>::iterator voxp = ballSpace.begin();
	{
		...
		while (vi != voxend)
		{
			findBoss(&*vi); /// find upper sphere in hirarchy
